...
double measure_flops(int num_iterations) 
{
    ...
    double flops = (num_iterations * 2) / elapsed_time;
     // Assuming one multiplication is one FLOP
...
{
    int t; long long index_t; size_t x;
};
...
extern int** matrixOutput(struct matrix mtix)
{
    int result[mtix.rows][mtix.cols] = {};
    for (int i = 1; i < mtix.rows; i++) 
    {
        for (int k = 1; k < mtix.cols; k++)
        {
            result[i][k] = i * k;
        }
    }
